...
%token LESSEQUAL LPAR LSQB MINEQUAL MINUS NAME NEWLINE NOT NOTEQUAL NUMBER NINT NFLOAT
...
%start start
//%token <ast> PLUSEQUAL MINEQUAL STAREQUAL SLASHEQUAL 
%type <ast> arith_expr term factor MINUS power pick_unop PLUS TILDE atom star_trailer pick_yield_expr_testlist  
%type <ast> DOUBLESTAR trailer testlist1 plus_STRING LPAR RPAR shift_expr star_EQUAL testlist print_stmt
%type <ast> expr_stmt opt_test 

%type <ast> yield_expr test star_COMMA_test opt_listmaker opt_dictorsetmaker or_test opt_IF_ELSE lambdef listmaker dictorsetmaker
%type <ast> and_test varargslist star_fpdef_COMMA pick_STAR_DOUBLESTAR fpdef opt_EQUAL_test star_COMMA_fpdef plus_stmt funcdef 
...
funcdef // Used in: decorated, compound_stmt
	: DEF NAME parameters COLON suite {	fname=$2;
						TableManager t;//cout<<"entered"<<endl;
						t.insert(fname,scope);//cout<<"exited"<<endl;
						scope=0;
...
parameters // Used in: funcdef
	...
	| LPAR RPAR {++count;scope=count;pc=scope;ic=scope;}//cout<<"Scope"<<scope<<endl;}
	;
...
opt_DOUBLESTAR_NAME // Used in: pick_STAR_DOUBLESTAR
	: COMMA DOUBLESTAR NAME //{std::cout<<">>> "<<$3<<std::endl;}
	| %empty
	;
...
expr_stmt // Used in: small_stmt
	: testlist augassign pick_yield_expr_testlist  {
								if ($2=='+') {$$=new Add($1,$3);}
								else if ($2=='-') {$$=new Sub($1,$3);}
								else if ($2=='*') {$$=new Mul($1,$3);}
								else if ($2=='/') {$$=new Divide($1,$3);}
								else if ($2=='%') {$$=new Mod($1,$3);}
								else if ($2=='^') {$$=new Power($1,$3);}
								else if ($2=='#') {$$=new Dslash($1,$3);}
								SymbolTable s;   	
								xst=$1->getName();
								stringstream ss;
                                               			ss<<scope;
	...
	| testlist star_EQUAL {
                                        if($2!=NULL) {
					SymbolTable s;//=Symbol::getInstance();	
					xst=$1->getName();
					stringstream ss;
                                        ss<<scope;
					xst=xst+ss.str(); 
...
print_stmt // Used in: small_stmt
	: PRINT opt_test /*{dc=$2;std::cout<<">>> "<<compute(dc)<<std::endl;}*/{if (check==scope){std::cout<<">>> "<<compute($2)<<std::endl;treeFree($2);}
			   else { if(scope==pc){Pvalue p;p.insert(scope,$2);} else {ic=ic+20;Pvalue p;p.insert(ic,$2);} } ++pc;}
...
arith_expr // Used in: shift_expr, arith_expr
	: term
	| arith_expr pick_PLUS term { $$ = new Add($1,$3); }
	  //{$$=$1+$3;}	  
	| arith_expr pick_MINUS term{ $$ = new Sub($1,$3); }
	  //{$$=$1-$3;}
	;
pick_PLUS// Used in: arith_expr
	: PLUS	
	;
pick_MINUS// Used in: arith_expr
	: MINUS	
	;
term // Used in: arith_expr, term
	: factor
	| term STAR factor		{ $$ = new Mul($1,$3); }//{$$=$1*$3;}
	| term SLASH factor		{ $$ = new Divide($1,$3); }//{if ($3==0) {std::cout<<"DIVIDE BY ZERO ERROR"<<std::endl;YYACCEPT;}
					 //else if (($1<0&&$3>0)&&($1%$3!=0))
					//	 $$=$1/$3-1;
					//	else if (($1>0&&$3<0)&&($1%$3!=0))
					//	 $$=$1/$3-1;
					//	     else $$=$1/$3;}

	| term PERCENT factor		{ $$ = new Mod($1,$3); }//{if ($3==0) {std::cout<<"DIVIDE BY ZERO ERROR"<<std::endl;YYACCEPT;}
					//else $$=$1%$3;
					//if(($1<0&&$3>0)&&($1%$3!=0)) $$=$3+$$;
					//   else if(($1>0&&$3<0)&&($1%$3!=0)) $$=$3+$$;
					//}
	| term DOUBLESLASH factor        {$$= new Dslash($1,$3);}
			    
	;


factor // Used in: term, factor, power
	: MINUS factor	{ $$ = new Umin($2, NULL); }//{$$=-$2;}
	| PLUS factor   { $$ = new Uplus($2, NULL); }
	| power		{$$=$1;}
	| pick_unop factor
	;
...
power // Used in: factor
	: atom star_trailer DOUBLESTAR factor	{ $$ = new Power($1,$4); }//{$$=pow($1,$4);if ($4<0) std::cout<<"Negative exponentional value is not a part of project and hence the output will not be accurate"<<std::endl; }
	| atom star_trailer			{
							if ($2!=NULL)
							{
								//cout<<"fcall ="<<cname<<endl;
								TableManager t;
								bool m=t.search(cname);
								if (m==true)
								{
								int z=t.getvalue(cname);
								//cout<<"found ="<<z<<endl;
								scope=z;
								Pvalue v;
								int n=1;
								...
								else {cout<<"function not declared ignore any displayed value"<<endl;}
